...
//					}
					...
					String param = paraValues[0];
                    Date value = DateUtil.getDate(param);
					field.set(obj, value);
				} else if (Timestamp.class.equals(field.getType())) {
					String param = paraValues[0];
                    Date value = DateUtil.getTime(param);
					if (null != value) {
...
//						}
					...
					for (int i = 0; i < paraValues.length; i++) {
						String param = paraValues[i];
                        Array.set(array, i, DateUtil.getDate(param));
					}
					field.set(obj, array);
				} else if (Timestamp[].class.equals(field.getType())) {
...
//							}
						...
						for (int i = 0; i < paraValues.length; i++) {
							String param = paraValues[i];
                            list.add(DateUtil.getDate(param));
						}
						field.set(obj, list);
